...
class UserController extends Controller
{
    ...
    {
        $transformedData = [
            'info' => (!is_null($kycInfo->personal_information) || $kycInfo->kyc_level > 0) ? true : false,
            'selfie' => (!is_null($kycInfo->selfie) || $kycInfo->kyc_level > 2) ? true : false,
            'document' => (!is_null($kycInfo->document_front) || !is_null($kycInfo->document_back) || $kycInfo->kyc_level > 1) ? true : false,
            'proof_of_address' => (!is_null($kycInfo->proof_of_address) || $kycInfo->kyc_level > 3) ? true : false,
            'completed_verified' => $kycInfo->status == 'approved',
            'rejected' => $kycInfo->status == 'rejected',
            'reason' => $kycInfo->rejection_reason,
            'applicant_id' => $kycInfo->external_applicant_id,
            'data' => [
