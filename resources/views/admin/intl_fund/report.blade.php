@extends('admin.app')
@section('page_title', 'My Intl. Fund Transfer Orders')
@section('content')
    <div class="container-fluid">
        <div class="card">
            <div class="card-body">
                <h5 class="card-title fw-semibold mb-4">Intl. Fund Transfer Orders Reports</h5>
                ...
                <div class="d">
                    <form action="" method="get" class="from-inline">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-3">
                                <select name="pos" id="pos" class="selectize">
                                    <option value="">--select pos--</option>
                                    @foreach ($pos as $p)
                                        <option value="{{ $p->id }}">{{ str_pad($p->user_id, 4, '0', STR_PAD_LEFT) }} -
                                            {{ $p->name ?? '' }}({{ $p->business_name ?? '' }})</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-sm-3">
                ...
                @if (isset($reports))
                    ...
                    <div id="rep" class="p-1">
                        <h3>Intl Fund Transfer Orders</h3>
                        <table class="table table-borderless table-sm">
                            ...
                            <tr>
                                <th>Teller/ Pos: </th>
                                <td>{{ request()->get('pos') ? str_pad(request()->get('pos'), 4, '0', STR_PAD_LEFT) : 'All' }}
                                </td>
                            </tr>
                            <tr>
                                <th>
                            ...
                            <tbody>
                                ...
                                @foreach ($reports as $r)
                                    <tr>
                                        ...
                                        <td>{{explode('-',$r->rx_currency)[0]}} {{ number_format($r->rx_amount) }}</td>
                                        <td>
                                            @php
                                                $total_tt += $r->s_amount;
                                            @endphp
                                            {{explode('-',$r->s_currency)[0]}} {{ number_format($r->s_amount) }}
                                        </td>
                                        <td>
                                            @php
                                                $total_comm += $r->rate->commision;
                                            @endphp
                                            {{explode('-',$r->s_currency)[0]}} {{ number_format($r->rate->commision) }}
                                        </td>
                                        <td>
                                            @php
                                                $total_tax += 0;
                                            @endphp
                                            {{explode('-',$r->s_currency)[0]}} {{ 0.0 }}
                                        </td>
                                        <td>
                                            @php
                                                $total_total += $r->rate->commision + $r->s_amount;
                                            @endphp
                                            {{explode('-',$r->s_currency)[0]}} {{ number_format($r->rate->commision + $r->s_amount) }}
                                        </td>
                                    </tr>
                                @endforeach
                                <tr>
